 ...
 *
 *  This module DEFINES the bake and does not perform it. `scripts/build-neighbors.ts`
 *  is the entry point that calls it. That separation is not ceremony: this file
 *  used to call `main()` at module scope, and the freshness test imported it for
 *  a single constant — so the test re-baked the artifacts microseconds before
 *  asserting they were up to date. It could never fail, and test runs silently
 *  rewrote `src/generated/`. A module that both defines work and performs it on
 *  import cannot be safely depended on.
 *
...
/** Resolved from the repo root rather than from this file, so moving this
 *  module cannot silently redirect the write. It already did once: relocating
 *  this file one directory deeper turned `../src/generated/` into
 *  `scripts/src/generated/`, and the bake reported success while writing
...
export function buildNeighbors(): void {
